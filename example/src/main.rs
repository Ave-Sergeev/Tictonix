...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let token_embedding = embeddings_uniform.get_embedding(tokens[0])?;
    println!("Token Embedding:\n{}\n", token_embedding);

    // Update the embedding for a specific token (index) in the embedding matrix
    let _ = embeddings_uniform.update_embedding(tokens[1], token_embedding)?;
